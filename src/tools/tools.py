...
async def register_service(id: str, name: str, address: str, port: int, consul_url: str = "http://localhost:8500") -> Dict:
    ...
    try:
        ...
        return {"status": "error", "message": f"HTTP error: {e.response.status_code}", "service_id": id}
    except Exception as e:
        return {"status": "error", "message": f"Failed to register service: {str(e)}", "service_id": id}
...
async def deregister_service(id: str, consul_url: str = "http://localhost:8500") -> Dict:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            r.raise_for_status()
            return {"status": "success", "message": f"Service deregistered successfully", "service_id": id, "http_status": r.status_code}
    except httpx.HTTPStatusError as e:
        return {"status": "error", "message": f"HTTP error: {e.response.status_code}", "service_id": id}
    except Exception as e:
        return {"status": "error", "message": f"Failed to deregister service: {str(e)}", "service_id": id}
...
async def get_service_health(service_name: str, consul_url: str = "http://localhost:8500") -> Dict:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if not health_data:
                return {"status": "not_found", "message": f"Service '{service_name}' not found", "service_name": service_name}
            
            all_healthy = all(all(check.get("Status") == "passing" for check in instance.get("Checks", [])) for instance in health_data)
                ...
                "instances": [
                    {
                        ...
                        "checks": [{"name": check["Name"], "status": check["Status"]} for check in inst.get("Checks", [])]
                    }
                    for inst in health_data
                ]
            }
    except httpx.HTTPStatusError as e:
        return {"status": "error", "message": f"HTTP error: {e.response.status_code}", "service_name": service_name}
    except Exception as e:
        return {"status": "error", "message": f"Failed to get service health: {str(e)}", "service_name": service_name}
...
async def create_intention(source: str, destination: str, action: str = "allow", consul_url: str = "http://localhost:8500") -> Dict:
    ...
    try:
        ...
        return {"status": "error", "message": f"HTTP error: {e.response.status_code}", "source": source, "destination": destination}
    except Exception as e:
        return {"status": "error", "message": f"Failed to create intention: {str(e)}", "source": source, "destination": destination}
...
async def delete_intention(intention_id: str, consul_url: str = "http://localhost:8500") -> Dict:
    ...
    try:
        ...
        return {"status": "error", "message": f"HTTP error: {e.response.status_code}", "intention_id": intention_id}
    except Exception as e:
        return {"status": "error", "message": f"Failed to delete intention: {str(e)}", "intention_id": intention_id}
...
async def get_intention(intention_id: str, consul_url: str = "http://localhost:8500") -> Dict:
    ...
    try:
        ...
        if e.response.status_code == 404:
            return {"status": "not_found", "message": f"Intention '{intention_id}' not found", "intention_id": intention_id}
        return {"status": "error", "message": f"HTTP error: {e.response.status_code}", "intention_id": intention_id}
    except Exception as e:
        return {"status": "error", "message": f"Failed to get intention: {str(e)}", "intention_id": intention_id}
